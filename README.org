...
*** Codex (ChatGPT OAuth)
If you have a ChatGPT subscription via Codex, you can reuse the Codex CLI OAuth
tokens instead of an API key.

Configure the Codex CLI to store credentials in a file:
...
(For headless/remote setups, use =codex login --device-auth=.)

Use the cached tokens with gptel:

#+begin_src emacs-lisp
(require 'gptel)
(require 'gptel-codex)
(require 'gptel-codex-auth)

(setq gptel-backend
      (gptel-make-codex "Codex (ChatGPT OAuth)"
        :header #'gptel-codex-auth-header
        :models '(gpt-5.2-codex gpt-5.1-codex-mini gpt-5.1-codex-max gpt-5.2)))
...
Codex responses require streaming and curl support (set =gptel-stream= and
=gptel-use-curl= to non-nil).  The Codex backend sends model-specific prompts
from the bundled files in =codex-prompts/=; if you override these, set
=gptel-codex-instructions= to a valid Codex prompt string.
